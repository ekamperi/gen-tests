...
pics='DC_dtlb_L1_miss_L2_hit'          # DC L2 hits
pics=$pics,'DC_dtlb_L1_miss_L2_miss'   # DC L2 misses

/usr/bin/cputrack -c $pics,umask=31 -T 0.1 "$@" |
    awk -v quiet=$quiet 'BEGIN {
	if (!quiet) {
	    printf "%16s %16s %8s\n", "DC Hits", "DC Misses", "% Misses/Hits";
	}
	skipped = 0
    }
