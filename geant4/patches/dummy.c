...
static struct cb_ops dummy_cb_ops = {
	...
	dummy_close,
	nodev,
	/* no strategy - nodev returns ENXIO */
	nodev,
	/* no print */
	nodev,
	/* no dump */
	...
	/* no segmap */
	nochpoll,
	/* returns ENXIO for non-pollable devices */
	dummy_prop_op,
	NULL,
	/* streamtab struct; if not NULL, all above */
	/* fields are ignored */
	D_NEW | D_MP,
	/* compatibility flags: see conf.h */
	CB_REV,
	/* cb_ops revision number */
	nodev,
	/* no aread */
	nodev
	/* no awrite */
...
static struct dev_ops dummy_dev_ops = {
	...
	dummy_getinfo,
	nulldev,
	/* no identify - nulldev returns 0 */
	nulldev,
	/* no probe */
	dummy_attach,
	dummy_detach,
	nodev,
	/* no reset - nodev returns ENXIO */
	&dummy_cb_ops,
	(struct bus_ops *)NULL,
	nodev
	/* no power */
